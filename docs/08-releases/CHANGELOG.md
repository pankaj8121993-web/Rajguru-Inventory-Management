...
### Added — 2026-07-29 — Weighment entry and the role matrix

- **Weighment slips (Phase 4)** — manual entry with gross and tare typed and the net
  **generated by the database**, so a wrong net weight cannot be written by any path.
  All five weights preserved separately (DR-02)
- **Tolerance handling** — within 0.5% passes silently, beyond it asks for a reason,
  beyond 2% flags for approval. Both thresholds live in `system_settings` and change
  without a deployment (DR-03)
- **Duplicate detection** — nine-field comparison over a configurable window, raising a
  review that must be resolved explicitly. Nothing is merged or discarded (DR-05)
- **Status lifecycle** — draft → awaiting verification → verified, with a database guard
  preventing the entry user from verifying their own slip (INV-24) and preventing edits
  after posting (DR-06)
- **Roles and permissions (Phase 3)** — 21 roles, 50 permissions, 226 grants seeded from
  the draft permission matrix; scoped `user_roles` supporting the same role at several
  scopes; `user_has_permission()` with inheritance down the location hierarchy
- **Administration screen** — users and their scoped roles, the full role matrix as a
  grid, and the audit trail
- **Seed** — 7 weighment slips including an exact match, a within-tolerance difference,
  a beyond-tolerance one with a reason, an outward dispatch and one with no commodity yet
- **Tests** — 56 Vitest (was 38), 37 database assertions (was 22), 30 Playwright (was 19)

### Fixed

- A clean weighment save closed the form and took its confirmation with it, so the
  operator never saw the generated slip number they need to write on the paper. The
  confirmation now persists in the register.
- `date` columns were reaching React as JavaScript `Date` objects and crashing the
  weighment page. Dates are now cast to text at the query boundary.
